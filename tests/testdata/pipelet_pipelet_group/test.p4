...
control MyIngress(inout headers hdr,
    ...
    table tab_switch {
        key = {hdr.ipv4.srcAddr : exact;}
        actions = {common_act;decrement_ttl;increment_ttl;}
    }
    table tab21 {
        key = {hdr.ipv4.srcAddr : exact;}
        actions = {common_act;}
    ...
    apply {
        tab01.apply();
        if (hdr.ipv4.isValid()) { // node_3
            tab02.apply();
            tab03.apply();
            if (hdr.tcp.isValid()) { // node_6
                tab04.apply();
                tab05.apply();
                if (hdr.tcp.srcPort == 80) { // node_9
                    tab06.apply();
                }
            } else {
                tab07.apply();
                if (hdr.tcp.srcPort == 80) { // node_12
                    tab18.apply();
                    tab19.apply();
                }
                tab20.apply();
        ...
        if (hdr.tcp.srcPort == 80) { // node_24
            tab14.apply();
        } else if (hdr.tcp.srcPort == 100) { // node_26
            tab15.apply();
        } else if (hdr.tcp.srcPort == 120) { // node_28
            tab16.apply();
        } else {
            tab17.apply();
        }
