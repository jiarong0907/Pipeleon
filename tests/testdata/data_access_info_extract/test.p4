...
control MyIngress(inout headers hdr,
    ...
    action tab1_act(bit<16> port, bit<16> ecmp_base, bit<32> ecmp_count) {
        // runtime data assign
        hdr.tcp.srcPort = port;
        // complex expression
        hdr.migration.tabl1_data = (hdr.migration.tabl2_data >> 3) + port & 16;
        // constant assign
        meta.aaa = 1;
        // header field assign
        hdr.tcp.srcPort = hdr.tcp.dstPort;
        // hash
        hash(meta.aaa, HashAlgorithm.crc16,
	    ecmp_base,
	    { hdr.ipv4.srcAddr,
	      hdr.ipv4.dstAddr,
          hdr.ipv4.protocol,
